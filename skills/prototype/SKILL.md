...
2. **Trivial to run.** A UI prototype starts from one command in the project's existing task runner — `pnpm <name>`, `python <path>`, `bun <path>`, etc. A logic demo is a single HTML file the user can open directly. Either way, the user starts it without thinking.
